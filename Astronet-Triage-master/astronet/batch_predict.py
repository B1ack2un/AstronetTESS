...
#New function added to account for the memory usage of Matplotlib's plt function which causes
#an error with bitmapping when running in Windows 10.
def closePlots():
    plt.clf()
    plt.cla()
    plt.close("all")
...
def find_tce(ticid, sector):
    for filename in filenames:
        for record in tf.io.tf_record_iterator(filename):
            ex = tf.train.Example.FromString(record)
            if (ex.features.feature["tic_id"].int64_list.value[0] == ticid) \
                    and (ex.features.feature["Sectors"].int64_list.value[0] == sector):
                # print("Found {}_{} in file {}".format(tid, tce_plnt_num, filename))
                return ex
    raise ValueError("{} not found in files: {}".format(ticid, filenames))


def plot_tce(tid, sector, true, pred, save_dir='astronet/plots/'):
    #In addition to the closePlots() function this changes the backend of Matplotlib's
    #plot function to ensure continuous program performance.
    mpl.use("Agg")

    ex = find_tce(tid, sector)
    ...
    axes[0].patch.set_facecolor(color='0.8')
    axes[0].set_title('Global View')
    axes[0].grid(b=True, color='white')
    axes[0].set_xlabel('Time (mod period)')
    axes[0].set_ylabel('Normalized Brightness')
    axes[1].plot(local_view, ".", label=true + ' - ' + str(pred) + ', ' + str(sector))
    axes[1].patch.set_facecolor(color='0.8')
    axes[1].set_title('Local View')
    axes[1].grid(b=True, color='white')
    axes[1].set_xlabel('Time (mod period)')
    fig.suptitle('TIC '+str(tid), fontsize=24)
    plt.legend()
    plt.savefig(save_dir + str(tid) + '.png', bbox_inches='tight')
    closePlots()
...
def predict(model_dir, config, model_class, y_pred = defaultdict(list)):
    ...
    with tf.Session(graph=g) as sess:
        ...
        for filename in filenames:
            print(filename)
            for i, serialized_example in enumerate(tf.io.tf_record_iterator(filename)):
                prediction = sess.run(
                    model.predictions,
