...
for bad_list in bad_lists:
    ...
    new_dataframe['Sectors'] = int(bad_list.split('-')[6])
    new_dataframe['Disposition'] = 'J'
    bad_tces = pd.concat([bad_tces, new_dataframe], ignore_index=True)

bad_tces.to_csv(os.path.join(FLAGS.save_dir,'sector-%s-all.csv' % int(bad_list.split('-')[6])), index=False)
