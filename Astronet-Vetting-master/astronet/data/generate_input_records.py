...
can be used to train and evaluate models that classify TESS TCEs.
...
The downloaded CSV file should contain at least the following column names:
  ...
  tid: TESS ID of the target star.
...
The TESS light curves can be downloaded from the Mikulski Archive for Space
Telescopes (MAST) at:

  http://archive.stsci.edu/pub/kepler/lightcurves.

The TESS data is assumed to reside in a directory with the same structure as
the MAST archive. Specifically, the file names for a particular TESS target
star should have the following format:

    .../${tess_id:0:4}/${tess_id}/kplr${tess_id}-${quarter_prefix}_${type}.fits,

where:
  tess_id is the TESS id left-padded with zeros to length 9;
...
def _process_tce(tce):
  """Processes the light curve for a TESS TCE and returns an Example proto.
  ...
  Raises:
    IOError: If the light curve files for this TESS ID cannot be found.
  """
  ...
  g_v = preprocess.global_view(time, flux, tce.Period)
  l_v = preprocess.local_view(time, flux, tce.Period,
                                     tce.Duration)
  s_v = preprocess.secondary_view(time, flux, tce.Period, tce.Duration)
  ...
  _set_float_feature(ex, "global_view", g_v)
  _set_float_feature(ex, "local_view", l_v)
  _set_float_feature(ex, "secondary_view", s_v)
...
def make_eval_set(argv):
    ...
    for file_shard in file_shards:
        _process_file_shard(file_shard[0], file_shard[1])
    #]
    #pool.close()
